...
# 1. ECR Repository
########################################################
resource "aws_ecr_repository" "toto_ms_ex2_ecr_private_repo" {
  name = format("%s/%s", var.toto_env, "toto-ms-ex2")
}

########################################################
# 2. Task Definition
########################################################
resource "aws_ecs_task_definition" "toto_ms_ex2_service_task_def" {
  family = format("%s-%s", "toto-ms-ex2", var.toto_env)
  ...
  container_definitions = jsonencode([
    {
      name      = "toto-ms-ex2"
      image     = format("%s.dkr.ecr.%s.amazonaws.com/%s/%s:latest", data.aws_caller_identity.current.account_id, var.aws_region, var.toto_env, "toto-ms-ex2")
      environment = [
        {
            name = "HYPERSCALER", 
            value = "aws"
      ...
      logConfiguration = {
        ...
        options = {
          ...
          awslogs-group = format("/ecs/%s/%s", var.toto_env, "toto-ms-ex2")
...
# 3. Service
########################################################
resource "aws_ecs_service" "toto_ms_ex2_service" {
  name = "toto-ms-ex2"
  cluster = aws_ecs_cluster.ecs_cluster.arn
  task_definition = aws_ecs_task_definition.toto_ms_ex2_service_task_def.arn
  desired_count = 1
  ...
  load_balancer {
    target_group_arn = aws_lb_target_group.toto_ms_ex2_service_tg.arn
    container_name = "toto-ms-ex2"
    container_port = 8080
  }
}
...
########################################################
# 4.1. Cloud Build
resource "aws_codebuild_project" "toto_ms_ex2_container_builder" {
  name          = format("%s-%s", "toto-ms-ex2", var.toto_env)
  ...
  source {
    type     = "GITHUB"
    location = "https://github.com/nicolasances/toto-ms-ex2.git"
...
resource "aws_codepipeline" "toto_ms_ex2_ecs_pipeline" {
  name     = "toto-ms-ex2-ecs-pipeline-${var.toto_env}"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        FullRepositoryId = "nicolasances/toto-ms-ex2"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ProjectName = aws_codebuild_project.toto_ms_ex2_container_builder.name
      }
    }
  }
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        ServiceName = aws_ecs_service.toto_ms_ex2_service.name
...
########################################################
resource "aws_lb_target_group" "toto_ms_ex2_service_tg" {
  name = format("%s-tg-%s", "toto-ms-ex2", var.toto_env)
  port = 8080
  protocol = "HTTP"
...
##############################################################
resource "aws_lb_listener_rule" "toto_ms_ex2_alb_listener_rule" {
...
resource "aws_lb_listener_rule" "toto_ms_ex2_alb_listener_rule_https" {
