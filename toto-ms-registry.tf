...
# 1. ECR Repository
########################################################
resource "aws_ecr_repository" "toto_ms_registry_ecr_private_repo" {
  name = format("%s/%s", var.toto_env, "toto-ms-registry")
}

########################################################
# 2. Task Definition
########################################################
resource "aws_ecs_task_definition" "toto_ms_registry_service_task_def" {
  family = format("%s-%s", "toto-ms-registry", var.toto_env)
  ...
  container_definitions = jsonencode([
    {
      name      = "toto-ms-registry"
      image     = format("%s.dkr.ecr.%s.amazonaws.com/%s/%s:latest", data.aws_caller_identity.current.account_id, var.aws_region, var.toto_env, "toto-ms-registry")
      environment = [
        {
            name = "HYPERSCALER", 
            value = "aws"
      ...
      logConfiguration = {
        ...
        options = {
          ...
          awslogs-group = format("/ecs/%s/%s", var.toto_env, "toto-ms-registry")
...
# 3. Service
########################################################
resource "aws_ecs_service" "toto_ms_registry_service" {
  name = "toto-ms-registry"
  cluster = aws_ecs_cluster.ecs_cluster.arn
  task_definition = aws_ecs_task_definition.toto_ms_registry_service_task_def.arn
  desired_count = 1
  ...
  load_balancer {
    ...
    container_name = "toto-ms-registry"
    container_port = 8080
  }
}
...
########################################################
# 4.1. Cloud Build
resource "aws_codebuild_project" "toto_ms_registry_container_builder" {
  name          = format("%s-%s", "toto-ms-registry", var.toto_env)
  ...
  source {
    type     = "GITHUB"
    location = "https://github.com/nicolasances/toto-ms-registry.git"
...
resource "aws_codepipeline" "toto_ms_registry_ecs_pipeline" {
  name     = "toto-ms-registry-ecs-pipeline-${var.toto_env}"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        FullRepositoryId = "nicolasances/toto-ms-registry"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ProjectName = aws_codebuild_project.toto_ms_registry_container_builder.name
      }
    }
  }
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        ServiceName = aws_ecs_service.toto_ms_registry_service.name
...
########################################################
resource "aws_lb_target_group" "toto_ms_registry_service_tg" {
  name = format("%s-tg-%s", "toto-ms-registry", var.toto_env)
  port = 8080
  protocol = "HTTP"
...
##############################################################
resource "aws_lb_listener_rule" "toto_ms_registry_alb_listener_rule" {
...
resource "aws_lb_listener_rule" "toto_ms_registry_alb_listener_rule_https" {
